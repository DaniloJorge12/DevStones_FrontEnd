...
const PLACEHOLDER_IMAGEM =
    'https://t3.ftcdn.net/jpg/20/18/50/92/360_F_2018509289_R3iua3z2q2AS6I9cW7LcaCX6Xyflw35s.jpg';
...
const ui = {
    pt: {
        ...
        publicadoEm: (ano, autor) =>
            `Publicado em ${ano}, ${autor} apresenta uma narrativa marcada por conflitos sociais, tensões políticas e a organização da classe trabalhadora no Brasil dos anos 30.`,
    ...
    en: {
        ...
        publicadoEm: (ano, autor) =>
            `Published in ${ano}, ${autor} presents a narrative marked by social conflicts, political tensions, and the organization of the working class in 1930s Brazil.`,
        resumo: 'Work Summary',
        ...
        async function carregarLivro() {
            try {
                ...
                if (!livroRes.ok || !personagensRes.ok) {
                    throw new Error(
                        `HTTP error: livro=${livroRes.status} personagens=${personagensRes.status}`,
                    );
                }
                ...
                const livroAPI =
                    lista.find((l) => Number(l.id) === ID_LIVRO_CAMINHO_PEDRAS) ?? lista[0];
                ...
                setLivro({
                    ...
                    analise_en:
                        livroAPI.caracteristicasLiterarias_en ??
                        livroAPI.caracteristicasLiterarias ??
                        'No analysis',
            ...
            <div className="conteudoLivro__inner">
                <section className="heroLivro">
                    <div className="imagemLivro">
                        <img
                            src={livro.capa || PLACEHOLDER_IMAGEM}
                            alt={livro.capa ? livro.titulo : ''}
                            onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = PLACEHOLDER_IMAGEM;
                                e.target.alt = '';
                            }}
                        />
                    </div>
                    ...
                    <div className="infoLivro">
                        ...
                        <p className="descricaoLivro">{t.publicadoEm(livro.ano, livro.autor)}</p>
                    </div>
                </section>
